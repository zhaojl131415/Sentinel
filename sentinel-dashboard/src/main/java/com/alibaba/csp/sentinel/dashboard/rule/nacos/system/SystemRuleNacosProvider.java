...
//                .collect(Collectors.toList());
        ...
        if (list.isEmpty()) {
            ...
            // 解析json获取到 List<SystemRule>
            List<SystemRule> ruleList = JSON.parseArray(rules, SystemRule.class);
            // SystemRule------->SystemRuleEntity
            return ruleList.stream().map(rule ->
                            SystemRuleEntity.fromSystemRule(machine.getApp(), machine.getIp(), machine.getPort(), rule))
                    .collect(Collectors.toList());
        }
    }
