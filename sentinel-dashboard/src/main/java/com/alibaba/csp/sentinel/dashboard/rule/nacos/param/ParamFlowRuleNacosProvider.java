...
//                .collect(Collectors.toList());
        ...
        if (list.isEmpty()) {
            ...
            // 解析json获取到 List<ParamFlowRule>
            List<ParamFlowRule> ruleList = JSON.parseArray(rules, ParamFlowRule.class);
            // ParamFlowRule------->ParamFlowRuleEntity
            return ruleList.stream().map(rule ->
                            ParamFlowRuleEntity.fromParamFlowRule(machine.getApp(), machine.getIp(), machine.getPort(), rule))
                    .collect(Collectors.toList());
        }
    }
}