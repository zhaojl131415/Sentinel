...
//                .collect(Collectors.toList());
        ...
        if (list.isEmpty()) {
            ...
            // 解析json获取到 List<AuthorityRule>
            List<AuthorityRule> ruleList = JSON.parseArray(rules, AuthorityRule.class);
            // AuthorityRule------->AuthorityRuleEntity
            return ruleList.stream().map(rule ->
                            AuthorityRuleEntity.fromAuthorityRule(machine.getApp(), machine.getIp(), machine.getPort(),rule))
                    .collect(Collectors.toList());
        }
    }
